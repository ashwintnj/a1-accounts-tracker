...
const MonthlySummaryPage = () => {
    ...
    const handleCopyRow = async (e, item) => {
        e.stopPropagation(); // Prevents navigating if the row is clickable

        const expense = item.todayExpense ?? item.expense ?? 0;
        const sales = item.calculatedSales ?? item.sales ?? 0;
        const cih = item.cih ?? item.cashInHand ?? 0;
